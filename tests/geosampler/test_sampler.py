...
from geosampler.core.types import URI
...
def test_grid_sampling(tile_size: int, bounds: List, crs: str,
    ...
    assert isinstance(output[0], gpd.GeoDataFrame)
    assert isinstance(output[1], gpd.GeoDataFrame)
...
# n_sample, mask, field_name, min_percentage, max_percentage,
test_weighted_cyclic_sampling_data: List = [
    (64, t_bounds, test_crs, None, 0, False, "intersects", True),
    (32, None, None, str(path_to_data), 0, True, "intersects", False),
    (128, None, test_crs, geodf, 15, True, "within", True)
]
...
def test_systematic_sampling(
    ...
    weights: Union[str, List, None] = None,
    interval: int = 1,
